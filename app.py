...
def get_products_by_category(category):
    filtered = {k: v for k, v in products.items() if category.lower() in k.lower()}
...
def ai_search():
    ...
    product_names = {k: v["name"].lower() for k, v in products.items()}
    matches = get_close_matches(query, product_names.values(), n=5, cutoff=0.3)
    ...
    return jsonify(results if results else {'message': 'No products found'})
